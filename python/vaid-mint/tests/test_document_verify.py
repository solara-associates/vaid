...
def test_third_party_verifies_with_public_key_only():
    ...
    assert verify_vaid_document(public_key, vaid), "a genuine VAID verifies under the public key alone"
...
def test_tampered_document_fails():
    ...
    assert not verify_vaid_document(public_key, forged), "a rewritten field must fail"


def test_a_different_key_does_not_verify():
    _pk, vaid = _public_key_and_doc()
    other = ReferenceIssuer.ephemeral(1).kernel_public_key()
    assert not verify_vaid_document(other, vaid), "another issuer's key must not verify it"
...
def test_verifies_the_frozen_mint_vector_with_public_key_only():
    ...
    assert verify_vaid_document(public_key, doc), "the frozen mint vector verifies under its public key alone"

    bad = dict(v["input"])
    sig = bytearray(bytes.fromhex(v["ed25519"]["signature_hex"]))
    sig[0] ^= 0x01
    bad["kernel_signature"] = list(sig)
    assert not verify_vaid_document(public_key, bad), "a one-byte signature flip must fail"
