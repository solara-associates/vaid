...
fn mint(issuer: &ReferenceIssuer, class: &str, parent: Option<VaidId>) -> Vaid {
    issuer
        .issue_vaid_with_lineage(
            ...
            vec![],
            vec![],
            // No parent-expiry ceiling: these fixtures exercise REVOCATION across a
            // restart, and every VAID here is minted under the same 24h issuer TTL.
            // Expiry containment is covered by chain_expiry_v1.json.
            None,
        )
        .expect("mint")
}
