---
name: vaid
description: Mint, present, verify and revoke VAIDs (Verifiable Agent Identity) — short-lived, cryptographically signed credentials that say which agent is acting, for which tenant, within which scope, with which capabilities, and under whose delegation. Use when an agent needs to prove what it is to another system or agent, when you receive a credential from an agent and need to check it, or when delegating a narrowed subset of your authority to a sub-agent. Also use when the user mentions VAID, agent identity, agent attestation, capability attenuation, or asks "how do I prove this agent is allowed to do that".
license: Apache-2.0
---
