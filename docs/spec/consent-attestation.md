...
**Status:** implemented, format **NOT frozen**
...
## C.5 A time bound is not withdrawal
...
## C.7 Replayed and absent consent share a verdict
...
## C.8 What is deliberately not checked
...
The attestation format is **not frozen** and no conformance vector exists for it.
Byte agreement across Rust, Python and TypeScript is proven instead by
`scripts/attestation_byte_agreement.sh`, which emits the same fixed attestation from
all three implementations and diffs them. When the format is approved this becomes a
vector under the usual three-way `cmp` discipline, and `chain_v1.json` — which
contains no attestations — is unaffected either way.
